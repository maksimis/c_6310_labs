...
void	test_ft_memset_bzero(void)
{
	...
	char buf[10];
	ft_memset(buf, 'A', 5);
	printf("ft_memset: first 5 bytes = %c%c%c%c%c\n", buf[0], buf[1], buf[2], buf[3], buf[4]);
	ft_bzero(buf, 3);
	printf("ft_bzero: first 3 bytes = %d,%d,%d\n", buf[0], buf[1], buf[2]);
}
...
void	test_ft_strnstr(void)
{
	...
	printf("ft_strnstr(\"%s\", \"Hello\", 3) = %s (expected: NULL)\n", big, result ? result : "NULL");
}
...
void	test_ft_lstdelone(void)
{
	...
	printf("After ft_lstdelone: element deleted (memory freed)\n");
}
