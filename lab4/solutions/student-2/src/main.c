...
static int	match(const char *str, int criterion, ...)
{
	va_list	args;
	int		n;
	char	*prefix;
...
{
	return (match(str, CRIT_START, (char *)param));
}
...
{
	t_filter	filters[] = {
		{"len", CRIT_LEN, 1, pred_len},
		{"start", CRIT_START, 1, pred_start},
		{"digit", CRIT_DIGIT, 0, pred_digit},
		{NULL, 0, 0, NULL}
	};
	t_filter	*filter;
	char		**lines;
